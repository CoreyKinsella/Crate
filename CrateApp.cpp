...
void CrateApp::BuildRootSignature()
{
	...
	texTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0); // Support 2 textures instead of 1
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	srvHeapDesc.NumDescriptors = 2; // 2 textures instead of 1
	...
	hDescriptor.Offset(1, md3dDevice->GetDescriptorHandleIncrementSize(srvHeapDesc.Type));
	//Increment this
	//Alpha Resource
	auto flareAlphaTex = mTextures["flareAlphaTex"]->Resource;
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc1 = {};
	srvDesc1.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc1.Format = flareAlphaTex->GetDesc().Format;
	srvDesc1.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc1.Texture2D.MostDetailedMip = 0;
	srvDesc1.Texture2D.MipLevels = flareAlphaTex->GetDesc().MipLevels;
	srvDesc1.Texture2D.ResourceMinLODClamp = 0.0f;
	md3dDevice->CreateShaderResourceView(flareAlphaTex.Get(), &srvDesc1, hDescriptor);
}
...
{
	auto flareRitem = std::make_unique<RenderItem>();
	flareRitem->ObjCBIndex = 0;
	flareRitem->Mat = mMaterials["flare"].get();
	flareRitem->Geo = mGeometries["boxGeo"].get();
	flareRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	flareRitem->IndexCount = flareRitem->Geo->DrawArgs["box"].IndexCount;
	flareRitem->StartIndexLocation = flareRitem->Geo->DrawArgs["box"].StartIndexLocation;
	flareRitem->BaseVertexLocation = flareRitem->Geo->DrawArgs["box"].BaseVertexLocation;
	mAllRitems.push_back(std::move(flareRitem));
